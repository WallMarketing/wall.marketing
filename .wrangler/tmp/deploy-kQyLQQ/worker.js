...
__name(isAuthorizedDevice, "isAuthorizedDevice");
...
async function verifyStripeSignature(payload, signature, secret) {
  ...
  if (!Number.isFinite(timestamp) || !received || Math.abs(Date.now() / 1e3 - timestamp) > 300) return false;
...
__name(deviceTokenFromRequest, "deviceTokenFromRequest");
...
__name(makeEmailMessage, "makeEmailMessage");
...
__name(sendSuccessEmailForOrder, "sendSuccessEmailForOrder");
var worker_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/checkin") {
      ...
      const issuedToken = presentedToken ? null : crypto.randomUUID().replaceAll("-", "") + crypto.randomUUID().replaceAll("-", "");
      const issuedTokenHash = issuedToken ? await sha256Hex(issuedToken) : null;
      const toIntOrNull = /* @__PURE__ */ __name((v) => {
    ...
    if (url.pathname === "/api/location-details" && request.method === "GET") {
      ...
      const components = Object.fromEntries((details.result.address_components || []).flatMap(
        (component) => component.types.map((type) => [type, component.long_name])
      ));
      return jsonResponse({
    ...
    if (url.pathname === "/api/orders" && request.method === "POST") {
      ...
      await env.DB.prepare(
        ...
        totalUsd,
        invoiceRequired ? 1 : 0,
        invoiceDetails.businessName || null,
        invoiceDetails.businessRegistrationNumber || null,
        invoiceDetails.taxNumber || null,
        invoiceDetails.contactName || null,
        invoiceDetails.phone || null,
        invoiceDetails.address || null,
        invoiceDetails.city || null,
        invoiceDetails.postcode || null,
        invoiceDetails.country || null
      ).run();
...
You can return to checkout to try again or contact us for help.`
    ...
    if (scheduleActionMatch && request.method === "POST") {
      ...
      if (action === "clear") {
        ...
        return result2.meta.changes ? jsonResponse({ ok: true }) : jsonResponse({ error: "schedule not found" }, 404);
      }
    ...
    if (deviceUpdateMatch && request.method === "PATCH") {
      ...
      if (!Number.isFinite(dailyCostUsd) || dailyCostUsd < 0 || dailyCostUsd > 1e5 || Math.round(dailyCostUsd * 100) !== dailyCostUsd * 100) {
      ...
      if (siteLocation) {
        ...
        const result2 = await geocodeResponse.json();
        if (result2.status === "ZERO_RESULTS" || !result2.results?.length) {
        ...
        latitude = Number(result2.results[0].geometry.location.lat);
        longitude = Number(result2.results[0].geometry.location.lng);
